// ===================================
// ­ƒÜÇ PERGUNU EXPRESS.JS API SERVER
// ===================================
...
const corsOptions = {
  origin: (origin, callback) => {
    // Log for debugging
    console.log('­ƒöì CORS check - Origin:', origin, 'NODE_ENV:', process.env.NODE_ENV);
    ...
    if (!origin) {
      console.log('Ô£à CORS: Allowing same-origin request (no origin header)');
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (isLocalhost) {
          console.log('Ô£à CORS: Allowing localhost origin');
          return callback(null, true);
        }
      }
      ...
      console.log('­ƒöì CORS allowList:', Array.from(allowList));
      
      if (allowList.has(origin)) {
        console.log('Ô£à CORS: Origin in allowlist');
        return callback(null, true);
      }
      
      // In production, be more permissive with Vercel preview URLs
      if (origin.includes('vercel.app')) {
        console.log('Ô£à CORS: Allowing Vercel domain');
        return callback(null, true);
      }
      
      // Log rejected origins for debugging
      console.warn(`ÔØî CORS: Rejected origin ${origin}`);
      
    } catch (error) {
      console.error('ÔØî CORS: Invalid origin URL:', error.message);
    }
  ...
  methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'], // Ô£à Added PATCH
...
app.get('/api/news/events', (req, res) => {
  console.log('­ƒöö New SSE client connected');
  ...
  req.on('close', () => {
    console.log('­ƒöö SSE client disconnected');
    sseClients.delete(res);
  });
});

// ===== DATABASE FUNCTIONS =====
// LOCAL JSON ONLY - MongoDB/Vercel/Cloudinary DISABLED
let useMongoDB = false;  // Always false - using local JSON only

console.log('📄 Using LOCAL JSON file only (MongoDB, Vercel, and Cloudinary disabled)');

/* MongoDB connection code disabled
  ...
  while (retries < maxRetries && !mongoInitialized) {
    try {
      ...
      if (db) {
        ...
        console.log('­ƒìâ Using MongoDB for persistent storage');
        
        // Test connection by listing collections
        const collections = await db.listCollections().toArray();
        console.log('­ƒôè Available collections:', collections.map(c => c.name).join(', ') || 'none');
        
        // Count documents in each collection
        for (const coll of ['users', 'news', 'applications', 'beasiswa']) {
          try {
            const count = await db.collection(coll).countDocuments();
            console.log(`  ÔÇó ${coll}: ${count} documents`);
          } catch (e) {
            console.log(`  ÔÇó ${coll}: collection not found`);
          }
        }
        break;
      } else {
        console.log('ÔÜá´©Å MongoDB connection returned null, retrying...');
      }
    } catch (error) {
      console.error(`ÔØî MongoDB initialization error (attempt ${retries + 1}):`, error.message);
    }
...
// LOCAL JSON ONLY - No Vercel /tmp usage
const DB_PATH = join(__dirname, 'db.json');
console.log('📂 Database path:', DB_PATH);

// Helper: Get collection from LOCAL JSON file
...
const readDB = () => {
  try {
    ...
    if (!existsSync(DB_PATH)) {
      console.log('\u26a0\ufe0f Database file not found, creating default structure...');
      const defaultDB = {
        users: [],
        news: [],
        sessions: [],
...
app.get('/api/news', async (req, res) => {
  try {
    ...
    console.log('­ƒô░ Returning', news.length, 'news items');
    res.json(news || []);
  } catch (error) {
...
app.post('/api/news', async (req, res) => {
  try {
    ...
    console.log('­ƒô░ Creating new news:', {
      title: newNews.title,
      hasImage: !!imagePath,
      imagePath: imagePath
    });
...
app.delete('/api/news/:id', async (req, res) => {
  try {
    ...
    console.log('­ƒùæ´©Å  Deleting news:', { id: req.params.id, title: existingNews.title });
...
app.post('/api/auth/login', async (req, res) => {
  try {
    console.log('­ƒöÉ Login attempt:', req.body);
    const users = await getCollection('users');
    ...
    if (!user) {
      console.log('ÔØî User not found:', username);
      return res.status(401).json({ error: 'Invalid credentials' });
    }
    
    console.log('­ƒæñ Found user:', user.username, 'Email:', user.email);
    
    const validPassword = await bcrypt.compare(password, user.password);
    if (!validPassword) {
      console.log('ÔØî Invalid password for user:', user.username);
      return res.status(401).json({ error: 'Invalid credentials' });
    }
    
    console.log('Ô£à Password valid for user:', user.username);
...
app.post('/api/auth/register', async (req, res) => {
  try {
    console.log('­ƒôØ Registration attempt:', req.body);
    const users = await getCollection('users');
    ...
    if (registeredEmailCheck) {
      console.log('ÔØî Registration blocked: Email already registered:', email);
...
app.get('/api/beasiswa', async (req, res) => {
  try {
    ...
    console.log('­ƒÄô Returning', beasiswaWithStatus.length, 'beasiswa items');
...
app.delete('/api/beasiswa/:id', async (req, res) => {
  try {
    ...
    console.log('­ƒùæ´©Å  Deleting beasiswa:', { id: req.params.id, judul: existing.judul });
...
app.get('/api/beasiswa/kategori/:kategori', async (req, res) => {
  try {
    ...
    console.log('­ƒÄô Returning', beasiswaWithStatus.length, 'beasiswa items for kategori:', kategori);
    res.json(beasiswaWithStatus);
  } catch (error) {
...
app.delete('/api/applications/:id', async (req, res) => {
  try {
    ...
    console.log(`­ƒùæ´©Å Deleting application: ${req.params.id}`);
...
app.patch('/api/applications/:id', async (req, res) => {
  try {
    ...
    console.log(`Ô£à Application ${req.params.id} updated:`, req.body.status || 'data updated');
...
app.get('/api/check-status/:email', async (req, res) => {
  try {
    ...
    console.log(`­ƒöì Checking application status for email: ${email}`);
  ...
  if (!application) {
    console.log(`ÔØî No application found for email: ${email}`);
    return res.json({
      success: false,
  ...
  console.log(`Ô£à Application found - Status: ${application.status}`);
...
app.get('/api/users', async (req, res) => {
  try {
    const users = await getCollection('users');
    console.log('­ƒæÑ Returning', users.length, 'users');
...
app.post('/api/upload/image', upload.single('image'), async (req, res) => {
  try {
    console.log('­ƒô© === Image Upload Request ===');
    console.log('File received:', req.file ? 'YES' : 'NO');
    
    if (!req.file || !req.file.buffer) {
      console.error('ÔØî No image file in request');
    ...
    console.log('­ƒô© Uploading image to Cloudinary:', {
    ...
    console.log('Ô£à Image uploaded successfully to:', cloudinaryUrl);
    ...
    console.error('ÔØî Error uploading image:', error);
...
app.get('/api/proxy-image', async (req, res) => {
  try {
    ...
    protocol.get(imageUrl, (imageResponse) => {
      ...
      console.error('ÔØî Proxy image error:', err);
      res.status(500).json({ error: 'Failed to proxy image' });
    });
  } catch (error) {
    console.error('ÔØî Proxy image error:', error);
    res.status(500).json({ error: 'Failed to proxy image' });
  }
});
...
app.get('/uploads/images/:filename', (req, res) => {
  console.log('ÔÜá´©Å Old image URL accessed:', req.params.filename);
  res.status(410).json({
...
// ===== ADMIN MIGRATION ENDPOINT (DISABLED - MongoDB Only) =====
/*
app.post('/api/admin/migrate', async (req, res) => {
  try {
    ...
    let db = getDB();
    if (!db) {
      console.log('­ƒöä MongoDB not connected, attempting to connect...');
      db = await connectDB();
      if (!db) {
    ...
    console.log('Ô£à Migration completed:', results);
    ...
    console.error('ÔØî Migration error:', error);
    res.status(500).json({ 
...
app.get('/api/admin/db-status', async (req, res) => {
  try {
    ...
    console.error('ÔØî DB status error:', error);
    res.status(500).json({ error: error.message });
  }
});

// POST /api/admin/clear-collection - Clear a specific collection (DISABLED - MongoDB Only)
/*
app.post('/api/admin/clear-collection', async (req, res) => {
  try {
    ...
    console.error('ÔØî Clear collection error:', error);
    res.status(500).json({ error: error.message });
  }
});
*/

// POST /api/admin/deduplicate - Remove duplicates from a collection (DISABLED - MongoDB Only)
/*
app.post('/api/admin/deduplicate', async (req, res) => {
  try {
    ...
    console.error('ÔØî Deduplicate error:', error);
    res.status(500).json({ error: error.message });
  }
});
*/
...
if (process.env.NODE_ENV !== 'production' && !process.env.VERCEL) {
  app.listen(PORT, () => {
    console.log('­ƒÜÇ ===== PERGUNU API SERVER STARTED =====');
    console.log(`­ƒôí Server running on http://localhost:${PORT}`);
    console.log(`­ƒöö SSE endpoint: http://localhost:${PORT}/api/news/events`);
    console.log('­ƒô░ News API endpoints:');
    ...
    console.log('­ƒîƒ Ready to serve!');
  });
}
